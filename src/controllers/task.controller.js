...
exports.listTasks = asyncHandler(async (req, res) => {
  ...
  if (req.user.role === 'admin') {
    ...
    allowedIds = allProjects.map((p) => p._id);
  } else {
    // Members only access joined projects
    ...
    allowedIds = userProjects.map((p) => p._id);
  }

  const filter = {
    project: { $in: allowedIds },
  };
  ...
  // Admin can filter assigned users
